...
monday delivers **`uninstall`** to this same monetization webhook (alongside `app_subscription_cancelled`). On an `uninstall` event the app calls `repository.purge_account()`, which **permanently deletes all of that account's data** — audit log, usage counters, and account row — in a single transaction. This satisfies the marketplace requirement to delete end-user data on uninstall (the deletion is immediate, well inside the 10-day window). The purge is idempotent and fail-open (a failure is logged/reported to Sentry but still acks 200, so monday doesn't retry-storm). Because these lifecycle/monetization events are signed with the app's **Client Secret** rather than the Signing Secret, the endpoint accepts a JWT that verifies under *either* app-owned secret.
