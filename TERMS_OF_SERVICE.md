...
The Application is an **informational screening tool**. When a screening is triggered for an item (a "vendor") on a connected monday.com board — by running the screening action, such as by clicking a button — the Application submits the vendor name, and optionally the vendor's country if you have mapped a country column, to the OpenSanctions data service, evaluates the response, and writes an indicative risk level (**Clear**, **Warning**, or **Critical**) together with supporting notes back to the board. On the paid plans, the Application also maintains an exportable audit trail of past screenings and sends you a notification when a vendor screens as Critical. Plans differ in their monthly screening allowance, the number of connected boards, and the available features, as presented on the marketplace listing (see Section 4).

## 2. Informational Purpose Only — No Decision-Making (IMPORTANT)

**2.1** The Application provides **information and indicative signals only**. It **does not make, and must not be relied upon to make, any decision** about any vendor, counterparty, transaction, or individual, including any decision to onboard, reject, continue, or terminate a business relationship.

**2.2** Output produced by the Application (including the "Clear", "Warning", and "Critical" labels and any accompanying text) is a **preliminary, automated indication** derived from third-party data. It is **not**: legal advice, regulatory advice, a compliance determination, a credit or background check, an adverse-media report, or confirmation of the identity of any entity.

**2.3** A "Clear" result **does not** mean a vendor is free of sanctions, PEP status, or other risk — it means no match was surfaced by the underlying data source at the time of the query. Data sources are incomplete, time-lagged, and may contain false positives and false negatives. Name-based matching in particular may return incorrect, partial, or ambiguous matches.

**2.4** **All decisions remain solely with the Customer.** The Customer is responsible for independently verifying any result, applying human judgment, and conducting such additional due diligence as its legal and regulatory obligations require.

## 3. Customer Responsibility for Compliance

**3.1** The Customer is solely responsible for its own compliance with all applicable laws and regulations, including anti-money-laundering (AML), know-your-customer (KYC), counter-terrorist-financing, sanctions, export-control, and data-protection laws. **Responsibility for regulatory compliance rests entirely with the Customer.**

**3.2** The Customer is responsible for determining whether the Application is suitable for its regulatory regime, for maintaining its own compliance program and records, and for ensuring that a qualified person reviews and acts upon any output.
